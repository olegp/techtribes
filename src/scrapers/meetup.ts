...
export default async function scrape(events: string | URL | Request) {
  ...
  const upcomingLink = $('a[href*="eventOrigin=group_upcoming_events"]')
    .first()
    .attr("href");
  const pastLink = $('a[href*="eventOrigin=group_past_events"]')
    .first()
    .attr("href");
  ...
  const location = $('[data-testid="group-info-location"]').text().trim();

  return {
    event:
      eventDate && cleanLink ? { date: eventDate, link: cleanLink } : undefined,
